# s21_RetailAnalytics_pytorch
![intro](img/Leonardo_Diffusion_XL_Create_an_image_illustrating_a_scene_whe_1.jpg)

## Summary
Проект представляет собой реализацию алгоритмов обработки данных в соответствии с приведенными ниже требованиями. 
    Всего приложение разбито на несколько частей:

    1. Terminal View. Обертка, через которую пользователь взаимодействует с приложением через ввод комманд в консоль. Terminal View отправляет команды в Контроллер 
    
    2. Controller. Контроллер, который выполняет переданные команды - проверяет данные, обрабатывает их и возвращает ответы
    
    3. Repository. Отсюда происходит основное общение с базой данных - контроллер дергает ручки репозитория, после чего репозиторий отправляет запросы в базу данных, которая лежит в файле dataframes.py и возвращает результаты запросов
    
    4. Dataframes. Только здесь создаются и хранятся все таблицы 
    
    5. Service / Static Service. Занимаются проверкой входных аргументов на их количество и соответствие типам. Разница между Service и Static Service заключается только в том, что метожы Service обращаются к базе данных, а методы Static Service - нет
    
    6. Types Validate. Содержит методы валидация типов данных под типы данных полей в базе данных
    
    7. Config. Содержит различные служебные сообщения 
    
    8. Part1, Part2. Здесь описаны правила создания таблиц и представлений
